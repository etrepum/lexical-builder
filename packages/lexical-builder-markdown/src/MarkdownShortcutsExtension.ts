...
function runTextMatchTransformers(
  ...
  for (const transformer of transformers) {
    ...
    let replaceNode: TextNode | undefined;

    if (startIndex === 0) {
      replaceNode = anchorNode.splitText(endIndex)[0];
    } else {
      replaceNode = anchorNode.splitText(startIndex, endIndex)[1];
    }
    invariant(
      $isTextNode(replaceNode),
      "MarkdownShortcuts: splitText(%s, %s)[%s] did not return the expected TextNode",
      String(startIndex),
      String(endIndex),
      String(startIndex === 0 ? 0 : 1),
    );
...
export const MarkdownShortcutsExtension = defineExtension({
  ...
  dependencies: [MarkdownTransformersExtension],
