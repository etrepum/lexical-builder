...
const editor = buildEditorFromPlans({
  // Any string is suitable as long as it uniquely defines the Plan in the editor
  name: "[root]",
...
const editor = buildEditorFromPlans({
  ...
  dependencies: [RichTextPlan, HistoryPlan],
  // highlight-end
  theme: { quote: "PlaygroundEditorTheme__quote" },
});
editor.setRootElement(editorRef);
...
export class KeywordNode extends TextNode {
  ...
  createDOM(config: EditorConfig): HTMLElement {
    ...
    dom.className = getPlanDependencyFromEditor(
      $getEditor(),
      KeywordsPlan,
    ).config.className;
    // highlight-end
