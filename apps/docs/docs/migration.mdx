...
const {editor} = buildEditorFromPlans({
  ...
  // highlight-start
  // RichTextPlan has a nodes property to add QuoteNode and HeadingNode
  // All three plans have register properties to add behavior to the editor,
...
import { ContentEditable } from "@lexical/react/LexicalContentEditable";
// highlight-start
import { AutoFocusPlan, HistoryPlan, RichTextPlan } from "@etrepum/lexical-builder";
...
<TabItem value="minimal" label="After (minimal & backwards compatible)">
...
<TabItem value="all-in" label="After (configurable without React dependency!)">
