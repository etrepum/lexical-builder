...
export const MarkdownTransformersExtension = defineExtension({
  name: "@etrepum/lexical-builder-markdown/MarkdownTransformers",
  dependencies: [RichTextExtension],
  peerDependencies: [
    declarePeerDependency<typeof CheckListExtension>(CHECK_LIST_PLAN_NAME),
  ],
  ...
  init(editorConfig, config, state): MarkdownTransformersOutput {
    ...
    // TODO: Awkward because CheckList is a separate extension from List
    //       but not a different node type!
    if (state.getPeer<typeof CheckListExtension>(CHECK_LIST_PLAN_NAME)) {
